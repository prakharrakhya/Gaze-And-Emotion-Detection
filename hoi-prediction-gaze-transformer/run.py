...
def main(opt):
    ...
    weight_path = path_new / Path(opt.weights)
    ...
    cfg = get_config(str(str(cfg_path)+"/final/"+f"/eval_hyp_f{future}.yaml"))
    ...
    with Path("hoi-prediction-gaze-transformer/vidhoi_related/obj_categories.json").open("r") as f:
        object_classes = json.load(f)
    with Path("hoi-prediction-gaze-transformer/vidhoi_related/pred_categories.json").open("r") as f:
        interaction_classes = json.load(f)
    with Path("hoi-prediction-gaze-transformer/vidhoi_related/pred_split_categories.json").open("r") as f:
